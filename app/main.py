...
# Password hashing (bcrypt directo, sin passlib para evitar conflicto con bcrypt 5.0)
pwd_context = None  # legacy, mantenido por compatibilidad

# OAuth2 para API (opcional, usamos cookies para web)
oauth2_scheme = OAuth2PasswordBearer(tokenUrl="/login", auto_error=False)

from app.database import (
    engine, SessionLocal, Base, init_db, get_db, seed_initial_data,
    Proveedor, IngredienteStock, RegistroCompra, DetalleCompra,
    RegistroMerma, ControlGasto, ListaVerificacionDiario,
    HigienePersonal, RegistroTemperatura,
    Usuario, Notificacion, Sucursal, Empresa, RolUsuario, TipoNotificacion,
    CategoriaIngrediente, UnidadMedida, EstadoAprobacion,
    TipoGasto, TipoMerma, TipoChecklist,
    Prioridad, Requerimientos
)
# ── FASE 4: OPERACIONES AVANZADAS ──
from app.database import (
    Alerta, Curso, CapacitacionUsuario, EjecucionChecklist, Evidencia, Incidencia,
    TipoAlerta, EstadoAlerta, EstadoCapacitacion,
    EstadoEjecucion, TipoIncidencia, EstadoIncidencia, Severidad,
    StockSucursal
)
# ── FASE 3: INVENTARIO FÍSICO ──
from app.database import (
    PlantillaInventario, ConteoFisico, ItemConteo, AjusteInventario, EstadoConteo
)

# Multi-Tenant: contexto de tenant por request + listener de filtrado global
from app.tenant import tenant_context, set_tenant, reset_tenant
...
async def crear_notificacion_y_telegram(
    db: Session,
    usuario_id: int,
    titulo: str,
    mensaje: str,
...
async def notificar_stock_critico(db: Session):
    ...
    for item in items_criticos:
        msg = (
            ...
            f"📦 <b>{item.nombre}</b>\n"
            f"Stock actual: {item.stock_actual} {item.unidad_medida.value}\n"
...
def calculate_dashboard_stats(db: Session, sucursal_id: int = None) -> dict:
    ...
    return {
        ...
        "mermas_pendientes": mermas_pendientes,
        "gastos_pendientes": gastos_pendientes,
        "checklist_hoy": checklist_hoy,
        "temp_fuera_rango": temp_fuera_rango,
...
@app.get("/inventario-fisico", response_class=HTMLResponse)
async def inventario_fisico_list(
    request: Request,
    db: Session = Depends(get_db),
    current_user: Usuario = Depends(require_encargado_or_admin)
):
    """Lista de plantillas de inventario y conteos recientes"""
    tenant_id = tenant_context.get()
    if not tenant_id:
        raise HTTPException(status_code=400, detail="Contexto de tenant no disponible")
    
    # Plantillas activas
    plantillas = db.query(PlantillaInventario).filter(
        PlantillaInventario.empresa_id == tenant_id,
        PlantillaInventario.activo == True
    ).all()
    
    # Conteos recientes (últimos 10)
    conteos = db.query(ConteoFisico).filter(
        ConteoFisico.empresa_id == tenant_id
    ).order_by(ConteoFisico.fecha.desc(), ConteoFisico.id.desc()).limit(10).all()
    
    # Cargar relaciones para mostrar en template
    for c in conteos:
        c.items_count = len(c.items) if c.items else 0
        c.diferencia_total = sum(i.diferencia_calculada for i in c.items) if c.items else 0
    
    return templates.TemplateResponse(
        request=request,
        name="inventario_fisico.html",
        context={
...
@app.get("/inventario-fisico/nuevo", response_class=HTMLResponse)
async def inventario_fisico_nuevo(
    request: Request,
    plantilla_id: Optional[int] = Query(None),
    db: Session = Depends(get_db),
    current_user: Usuario = Depends(require_encargado_or_admin)
):
    """Iniciar un nuevo conteo físico - selecciona plantilla o crea ad-hoc"""
    tenant_id = tenant_context.get()
    if not tenant_id:
        raise HTTPException(status_code=400, detail="Contexto de tenant no disponible")
    ...
    if plantilla_id:
        plantilla_seleccionada = db.query(PlantillaInventario).filter(
            PlantillaInventario.id == plantilla_id,
            PlantillaInventario.empresa_id == tenant_id
        ).first()
    
    sucursales = db.query(Sucursal).filter(
        Sucursal.empresa_id == tenant_id,
        Sucursal.activa == True
    ).all()
    
    return templates.TemplateResponse(
        request=request,
        name="inventario_fisico_nuevo.html",
        context={
            "plantillas": plantillas,
            "plantilla_seleccionada": plantilla_seleccionada,
            "sucursales": sucursales,
            "page_title": "Nuevo Conteo Físico"
        }
    )


@app.post("/inventario-fisico/crear")
async def inventario_fisico_crear(
    request: Request,
    fecha: str = Form(...),
    sucursal_id: int = Form(...),
    ...
    """Crear un nuevo conteo físico y sus items basados en plantilla o todos los ingredientes"""
    tenant_id = tenant_context.get()
    if not tenant_id:
        raise HTTPException(status_code=400, detail="Contexto de tenant no disponible")
    
    # Validar sucursal pertenece al tenant
    ...
    plantilla = None
    if plantilla_id:
        plantilla = db.query(PlantillaInventario).filter(
            PlantillaInventario.id == plantilla_id,
            PlantillaInventario.empresa_id == tenant_id,
            PlantillaInventario.activo == True
        ).first()
    ...
    fecha_conteo = datetime.strptime(fecha, "%Y-%m-%d").date()
    
    conteo = ConteoFisico(
        plantilla_id=plantilla_id,
        fecha=fecha_conteo,
        responsable_id=current_user.id,
        estado=EstadoConteo.EN_CONTEO,
        sucursal_id=sucursal_id,
    ...
    # Determinar qué ingredientes contar
    if plantilla:
        # TODO: En el futuro, la plantilla tendrá relación many-to-many con ingredientes
        # Por ahora, usar todos los ingredientes activos del tenant
        ingredientes = db.query(IngredienteStock).filter(
            IngredienteStock.empresa_id == tenant_id,
            IngredienteStock.activo == True
        ).all()
    else:
        ingredientes = db.query(IngredienteStock).filter(
    ...
    # Crear items con stock teórico actual
    for ing in ingredientes:
        # Obtener stock en la sucursal (StockSucursal) o usar stock global
        stock_suc = db.query(StockSucursal).filter(
            StockSucursal.ingrediente_id == ing.id,
            StockSucursal.sucursal_id == sucursal_id,
            StockSucursal.empresa_id == tenant_id
        ).first()
        stock_teorico = stock_suc.stock_actual if stock_suc else ing.stock_actual
        
        item = ItemConteo(
            conteo_id=conteo.id,
            ingrediente_id=ing.id,
            stock_teorico_sistema=stock_teorico,
            stock_real_contado=0.0,  # Se llenará en el conteo
            diferencia_calculada=0.0,  # Se calculará al guardar
            empresa_id=tenant_id
        )
        db.add(item)
    ...
    return RedirectResponse(url=f"/inventario-fisico/{conteo.id}", status_code=303)


@app.get("/inventario-fisico/{conteo_id}", response_class=HTMLResponse)
async def inventario_fisico_detalle(
    request: Request,
    conteo_id: int,
    db: Session = Depends(get_db),
    current_user: Usuario = Depends(require_encargado_or_admin)
):
    """Ver y editar un conteo físico (registrar stock real)"""
    tenant_id = tenant_context.get()
    if not tenant_id:
        raise HTTPException(status_code=400, detail="Contexto de tenant no disponible")
    
    conteo = db.query(ConteoFisico).filter(
        ConteoFisico.id == conteo_id,
        ConteoFisico.empresa_id == tenant_id
    ).first()
    if not conteo:
        raise HTTPException(status_code=404, detail="Conteo no encontrado")
    
    # Cargar items con ingredientes
    items = db.query(ItemConteo).filter(
        ItemConteo.conteo_id == conteo_id,
        ItemConteo.empresa_id == tenant_id
    ).all()
    
    for item in items:
        item.ingrediente = db.query(IngredienteStock).filter(
            IngredienteStock.id == item.ingrediente_id,
            IngredienteStock.empresa_id == tenant_id
        ).first()
        # Calcular diferencia en tiempo real para mostrar
        if item.stock_real_contado is not None:
            item.diferencia_calculada = item.stock_real_contado - item.stock_teorico_sistema
    
    return templates.TemplateResponse(
        request=request,
        name="inventario_fisico_detalle.html",
        context={
            "conteo": conteo,
            "items": items,
            "page_title": f"Conteo: {conteo.fecha}"
        }
    )


@app.post("/inventario-fisico/{conteo_id}/guardar-item")
async def inventario_fisico_guardar_item(
    request: Request,
    conteo_id: int,
    item_id: int = Form(...),
    stock_real: float = Form(...),
    db: Session = Depends(get_db),
    current_user: Usuario = Depends(require_encargado_or_admin)
):
    """Guardar stock real contado para un item (AJAX)"""
    tenant_id = tenant_context.get()
    if not tenant_id:
        raise HTTPException(status_code=400, detail="Contexto de tenant no disponible")
    
    item = db.query(ItemConteo).filter(
        ItemConteo.id == item_id,
        ItemConteo.conteo_id == conteo_id,
        ItemConteo.empresa_id == tenant_id
    ).first()
    if not item:
    ...
    item.stock_real_contado = stock_real
    item.diferencia_calculada = stock_real - item.stock_teorico_sistema
    db.commit()
    
    return {"success": True, "diferencia": item.diferencia_calculada}


@app.post("/inventario-fisico/{conteo_id}/finalizar")
async def inventario_fisico_finalizar(
    request: Request,
    conteo_id: int,
    accion: str = Form(...),  # "revision" o "cerrar"
    db: Session = Depends(get_db),
    current_user: Usuario = Depends(require_encargado_or_admin)
):
    """Finalizar conteo: pasar a REVISION o CERRADO (genera ajustes)"""
    tenant_id = tenant_context.get()
    if not tenant_id:
        raise HTTPException(status_code=400, detail="Contexto de tenant no disponible")
    ...
    if accion == "revision":
        conteo.estado = EstadoConteo.REVISION
        db.commit()
        return RedirectResponse(url=f"/inventario-fisico/{conteo_id}", status_code=303)
    
    elif accion == "cerrar":
        # Generar ajustes de inventario basados en diferencias
        items = db.query(ItemConteo).filter(
            ItemConteo.conteo_id == conteo_id,
            ItemConteo.empresa_id == tenant_id
        ).all()
        ...
        for item in items:
            if item.diferencia_calculada != 0:
                # Obtener stock actual en sucursal
                stock_suc = db.query(StockSucursal).filter(
                    StockSucursal.ingrediente_id == item.ingrediente_id,
                    StockSucursal.sucursal_id == conteo.sucursal_id,
                    StockSucursal.empresa_id == tenant_id
                ).first()
                
                stock_anterior = stock_suc.stock_actual if stock_suc else item.ingrediente.stock_actual
                stock_nuevo = item.stock_real_contado
                ...
                ajuste = AjusteInventario(
                    ...
                    valor_ajuste=stock_nuevo - stock_anterior,
                    responsable_id=current_user.id,
                    empresa_id=tenant_id
                )
                db.add(ajuste)
                ...
                if stock_suc:
                    stock_suc.stock_actual = stock_nuevo
                    stock_suc.updated_at = func.now()
                else:
                    # Crear registro StockSucursal si no existe
                    new_stock = StockSucursal(
                        ingrediente_id=item.ingrediente_id,
...
@app.get("/inventario-fisico/plantillas", response_class=HTMLResponse)
async def inventario_fisico_plantillas(
    ...
    plantillas = db.query(PlantillaInventario).filter(
        PlantillaInventario.empresa_id == tenant_id
    ).order_by(PlantillaInventario.fecha_creacion.desc()).all()
    
    return templates.TemplateResponse(
        request=request,
        name="inventario_fisico_plantillas.html",
        context={
            "plantillas": plantillas,
            "page_title": "Plantillas de Inventario"
        }
    )


@app.post("/inventario-fisico/plantillas/crear")
async def inventario_fisico_plantilla_crear(
    request: Request,
    nombre: str = Form(...),
    ...
    existing = db.query(PlantillaInventario).filter(
        PlantillaInventario.nombre == nombre,
        PlantillaInventario.empresa_id == tenant_id
    ).first()
    if existing:
    ...
    return RedirectResponse(url="/inventario-fisico/plantillas", status_code=303)
